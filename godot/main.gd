...
func _physics_process(delta):
    ...
    # Screen-relative movement: up on the stick always means up/forward on screen.
    ...
    # Camera follows position only. It no longer rotates when the survivor turns.
    if camera:
...
func _make_ui():
    ...
    var title := Label.new(); title.text = "BLACKOUT: SWEDEN  •  CONTROL TEST"; title.position = Vector2(22,18); title.add_theme_font_size_override("font_size",22); layer.add_child(title)
    var hint := Label.new(); hint.text = "Left thumb: move   •   Right thumb: interact"; hint.position = Vector2(22,50); layer.add_child(hint)

    # Movement belongs on the left; the right side is reserved for context actions.
    ...
    action_label = Label.new(); action_label.text = ""; action_label.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); action_label.position = Vector2(-300,-205); action_label.size = Vector2(260,45); action_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT; layer.add_child(action_label)

func _context_action():
    if action_label == null: return
    action_label.text = "Action button works ✓"
    var timer := get_tree().create_timer(1.2)
    timer.timeout.connect(func():
        if action_label: action_label.text = ""
    )
